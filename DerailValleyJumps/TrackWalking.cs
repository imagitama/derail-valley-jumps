...
public static class TrackWalking
{
    ...
    public static Junction? GetNextJunction(
        RailTrack current,
        Transform trainTransform)
    {
        ...
        while (iter != null && !visited.Contains(iter))
        {
            ...
            Junction j = direction ? iter.outJunction : iter.inJunction;
            if (j != null)
            {
                return j;
            }

            var next = iter.GetNextTrack(direction);
            if (next == null)
                break;

            direction = next.GetDirectionFromPrev(iter);
